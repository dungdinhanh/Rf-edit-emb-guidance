...
class HunyuanVideoAttnGuidancePipeline(DiffusionPipeline):
    """
    ...
    def __call__(
        ...
        video_length: int,
        inject_step: int,
        # emb_guidance_alpha not used in attn_guidance pipeline
        data_type: str = "video",
        ...
        attn_guidance_k: int = 32,
        attn_guidance_alpha: float = 1.0,
        ...
        **kwargs,
        ...
        # 3. Attention-level guidance: pass both cond/uncond separately
        #    Guidance is applied inside each attention layer, not here
        attn_guidance_config = {
            'uncond_text_states': uncond_embeds,
            'uncond_text_mask': uncond_mask,
            'uncond_text_states_2': uncond_embeds_2,
            'k': attn_guidance_k,
            'alpha': attn_guidance_alpha,
            'overlap_target': overlap_target,
        } if not inversion else None  # No guidance during inversion

        logger.info(
            f"Attention guidance: k={attn_guidance_k}, alpha={attn_guidance_alpha}, "
            f"overlap_target={overlap_target}, "
        ...
        latents = self.prepare_latents(
            ...
            cond_embeds.dtype, device, generator, latents,
        )
            ...
            for i, t in enumerate(timesteps[:-1]):
                ...
                with torch.autocast(
                    ...
                    noise_pred, info = self.transformer(
                        ...
                        t_expand,
                        info=info,
                        text_states=cond_embeds,
                        text_mask=cond_mask,
                        text_states_2=cond_embeds_2,
                        freqs_cos=freqs_cis[0],
                        freqs_sin=freqs_cis[1],
                        guidance=guidance_expand,
                        return_dict=True,
                        attn_guidance_config=attn_guidance_config,
                    )
