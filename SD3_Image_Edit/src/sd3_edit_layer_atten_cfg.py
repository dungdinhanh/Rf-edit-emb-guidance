"""
SD3 LayerAttenCFG — CFG applied to attention OUTPUT within each block.

Instead of running the full block twice, only the text QKV + attention
is computed twice (with cond and uncond text). The image attention outputs
are combined with CFG equation, then the residual + FF runs once.

    img_attn = img_attn_uncond + scale * (img_attn_cond - img_attn_uncond)
"""
...
class AttenCFGProcessor:
    """Custom attention processor that runs attention twice (cond/uncond) and combines."""

    def __init__(self, scale=1.0):
        self.scale = scale
        self.uncond_enc = None  # Set before each block forward
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None,
        ...
        if self.uncond_enc is not None and self.scale != 1.0:
            ...
            # Cond attention
            full_q_c = torch.cat([img_q, c_q], dim=2)
            full_k_c = torch.cat([img_k, c_k], dim=2)
            full_v_c = torch.cat([img_v, c_v], dim=2)
            attn_out_cond = F.scaled_dot_product_attention(full_q_c, full_k_c, full_v_c)

            # Uncond attention
            full_q_u = torch.cat([img_q, u_q], dim=2)
            full_k_u = torch.cat([img_k, u_k], dim=2)
            full_v_u = torch.cat([img_v, u_v], dim=2)
            attn_out_uncond = F.scaled_dot_product_attention(full_q_u, full_k_u, full_v_u)

            # CFG on image attention output
            img_len = residual.shape[1]
            img_attn_cond = attn_out_cond[:, :, :img_len, :]
            img_attn_uncond = attn_out_uncond[:, :, :img_len, :]
            img_attn_guided = img_attn_uncond + self.scale * (img_attn_cond - img_attn_uncond)

            # Text output from cond path
            txt_attn = attn_out_cond[:, :, img_len:, :]

            # Reshape
            img_attn_guided = img_attn_guided.transpose(1, 2).reshape(batch_size, img_len, inner_dim)
            txt_attn = txt_attn.transpose(1, 2).reshape(batch_size, -1, inner_dim)

            hidden_states_out = img_attn_guided.to(hidden_states.dtype)
            encoder_hidden_states_out = txt_attn.to(encoder_hidden_states.dtype)
        else:
            # Standard path (no guidance)
...
class SD3LayerAttenCFGEditor:
    ...
    def _install(self, neg_embeds, layer_scales):
        ...
        for i, block in enumerate(transformer.transformer_blocks):
            proc = AttenCFGProcessor(scale=layer_scales[i])
