"""PIE-Bench eval for SD3 per-layer embedding guidance."""
...
def main(args):
    ...
    method_name = f"perlayer_{args.schedule}_a{args.base_alpha}"
        ...
        for idx in range(n):
            ...
            try:
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           'category': cat, 'sample_id': sample_id, 'mode': 'perlayer',
                ...
                total_processed += 1
                if idx == 0:
                    alphas = info['layer_alphas']
                    print(f"  Alphas: [{alphas[0]:.2f}, ..., {alphas[11]:.2f}, ..., {alphas[23]:.2f}]")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--schedule', default='uniform',
                        choices=['uniform', 'linear_up', 'linear_down', 'peak_mid',
                                 'early_late', 'step_late', 'step_early'])
