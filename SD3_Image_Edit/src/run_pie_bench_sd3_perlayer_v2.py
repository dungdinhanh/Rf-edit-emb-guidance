"""PIE-Bench eval for SD3 per-layer v2 (parallel stream)."""
import os, json, time, io, argparse
import torch, numpy as np, pandas as pd
from PIL import Image
from sd3_edit_perlayer_v2 import SD3PerLayerV2Editor

def main(args):
    editor = SD3PerLayerV2Editor(model_id=args.model_id)
    ...
    method_name = f"perlayerv2_{args.schedule}_a{args.base_alpha}_ea{args.emb_alpha}"
        ...
        for idx in range(n):
            ...
            try:
                edited_img, elapsed, info = editor.edit(
                    ...
                    base_alpha=args.base_alpha, schedule=args.schedule,
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           'category': cat, 'sample_id': sample_id, 'mode': 'perlayer_v2',
                           'schedule': args.schedule, 'base_alpha': args.base_alpha,
                           'emb_alpha': args.emb_alpha,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--emb_alpha', type=float, default=1.0)
    parser.add_argument('--schedule', default='uniform',
                        choices=['uniform', 'linear_up', 'linear_down', 'peak_mid',
