"""
SD3 Three-Stage Editor with NORMALIZED Embedding Guidance.

Same three-stage approach as sd3_edit_3stage.py, but the embedding guidance
uses L2 norm matching to keep embeddings in distribution:

  v = (1 + alpha) * cond - alpha * uncond
  v_norm = v * (||cond|| / ||v||)   # per-token L2 normalize to ||cond||

This allows higher alpha without out-of-distribution collapse.
"""
...
class SD3ThreeStageNormEditor:
    ...
    def edit(self, source_image, source_prompt, target_prompt,
             ...
             emb_alpha=0.5, emb_early_alpha=None,
             cfg_scale=7.0,
             cfg_range=(0.3, 0.7), emb_range=(0.7, 1.0),
             emb_early_range=None):
        """
        Three-stage editing with normalized embedding guidance.

        Supports separate emb ranges and alphas for early and late.
        """
        t0 = time.time()
        ...
        # Prepare normalized guided embeddings (late)
