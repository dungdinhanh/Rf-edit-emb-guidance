"""PIE-Bench eval for SD3 three-stage with normalized embedding guidance."""
import os, json, time, io, argparse
import torch, numpy as np, pandas as pd
from PIL import Image
from sd3_edit_3stage_norm import SD3ThreeStageNormEditor

def main(args):
    editor = SD3ThreeStageNormEditor(model_id=args.model_id)
    ...
    method_name = f"3stage_norm_cfg{args.cfg_start}-{args.cfg_end}_emb{args.emb_start}-{args.emb_end}_a{args.emb_alpha}"
    if emb_early_range:
        ...
        for idx in range(n):
            ...
            try:
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           'category': cat, 'sample_id': sample_id, 'mode': '3stage_norm',
                           'emb_alpha': args.emb_alpha, 'emb_early_alpha': args.emb_early_alpha,
                           'cfg_scale': args.cfg_scale,
                ...
                total_processed += 1
                if idx == 0:
                    print(f"  Stages: none={info['none_steps']}, cfg={info['cfg_steps']}, emb_early={info['emb_early_steps']}, emb={info['emb_steps']}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--emb_alpha', type=float, default=0.7)
    ...
    parser.add_argument('--emb_start', type=float, default=0.5)
