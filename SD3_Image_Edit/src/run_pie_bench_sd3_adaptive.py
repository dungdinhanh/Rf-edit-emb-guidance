"""PIE-Bench evaluation using SD3 attention adaptive guidance (v3)."""
import os, sys, json, time, io, argparse
import torch, numpy as np, pandas as pd
from PIL import Image
from sd3_adaptive_guidance import SD3AdaptiveGuidanceEditor

def main(args):
    editor = SD3AdaptiveGuidanceEditor(model_id=args.model_id, offload=args.offload)
    ...
    method_name = f"adaptive_k{args.top_k}_d{args.drop_rate}"
        ...
        for idx in range(n):
            ...
            try:
                edited_img, elapsed = editor.edit(
                    ...
                    top_k=args.top_k, drop_rate=args.drop_rate,
                )
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           'category': cat, 'sample_id': sample_id, 'mode': 'adaptive_guidance',
                           'top_k': args.top_k, 'drop_rate': args.drop_rate,
