"""PIE-Bench eval for SD3 three-stage with conditional orthogonal embedding guidance."""
import os, json, time, io, argparse
import torch, numpy as np, pandas as pd
from PIL import Image
from sd3_edit_3stage_orth import SD3ThreeStageOrthEditor

def main(args):
    editor = SD3ThreeStageOrthEditor(model_id=args.model_id)
    ...
    method_name = f"3stage_orth_cfg{args.cfg_start}-{args.cfg_end}_emb{args.emb_start}-{args.emb_end}_a{args.emb_alpha}"
    if emb_early_range:
        ...
        for idx in range(n):
            ...
            try:
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           'category': cat, 'sample_id': sample_id, 'mode': '3stage_orth',
                           'emb_alpha': args.emb_alpha, 'emb_early_alpha': args.emb_early_alpha,
                           'cfg_scale': args.cfg_scale,
