"""PIE-Bench eval for SD3 per-layer CFG in hidden space."""
import os, json, time, io, argparse
import torch, numpy as np, pandas as pd
from PIL import Image
from sd3_edit_layercfg import SD3LayerCFGEditor

def main(args):
    editor = SD3LayerCFGEditor(model_id=args.model_id)
    ...
    method_name = f"layercfg_{args.schedule}_s{args.base_scale}"
        ...
        for idx in range(n):
            ...
            try:
                edited_img, elapsed, info = editor.edit(
                    ...
                    base_scale=args.base_scale, schedule=args.schedule,
                )
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           'category': cat, 'sample_id': sample_id, 'mode': 'layercfg',
                           'schedule': args.schedule, 'base_scale': args.base_scale,
