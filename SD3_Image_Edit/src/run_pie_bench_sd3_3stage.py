"""PIE-Bench evaluation using SD3 three-stage hybrid guidance."""
...
def main(args):
    editor = SD3ThreeStageEditor(model_id=args.model_id)
    ...
    method_name = f"3stage_cfg{args.cfg_start}-{args.cfg_end}_emb{args.emb_start}-{args.emb_end}_a{args.emb_alpha}_g{args.cfg_scale}"
        ...
        for idx in range(n):
            ...
            try:
                edited_img, elapsed, info = editor.edit(
                    ...
                    cfg_range=cfg_range, emb_range=emb_range,
                )
                ...
                json.dump({'source_prompt': source_prompt, 'target_prompt': target_prompt,
                           ...
                           'mode': '3stage', 'emb_alpha': args.emb_alpha,
                           'cfg_scale': args.cfg_scale,
                           'cfg_range': list(cfg_range), 'emb_range': list(emb_range),
                           'cfg_steps': info['cfg_steps'], 'emb_steps': info['emb_steps'],
                           'none_steps': info['none_steps'], 'total_steps': info['total_steps'],
                           'strength': args.strength, 'elapsed': elapsed},
                          open(os.path.join(out_dir, f"{sample_id}_meta.json"), 'w'), indent=2)
                total_processed += 1
                if idx == 0:
                    print(f"  Stages: none={info['none_steps']}, cfg={info['cfg_steps']}, emb={info['emb_steps']} (total={info['total_steps']})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg_start', type=float, default=0.3, help='CFG range start (fraction)')
    parser.add_argument('--cfg_end', type=float, default=0.7, help='CFG range end (fraction)')
    parser.add_argument('--emb_start', type=float, default=0.7, help='Emb range start (fraction)')
    parser.add_argument('--emb_end', type=float, default=1.0, help='Emb range end (fraction)')
